...
describe Sc::VLCPlayer do
  describe "#connect" do
    it "only tries once to restart the connection" do
      server     = double
    ...
    it "raises PlayerNotAvailableError if we make more than one attempt to restart the connection" do
      ...
      expect{ player.connect }.to raise_error(Sc::PlayerNotAvailableError)
    end
  end
end
